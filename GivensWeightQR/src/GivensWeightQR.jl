...
function last_inband_nonzero_index(
	...
	if row isa Colon && col < n
		...
		while ind > 0 && iszero(B[ind, col])
			ind -= 1
			iszero(ind) && thrown(error("last_inband_nonzero_index: No nonzero element in column $col \n"))
		end
	elseif col isa Colon && row < m
		ind = last_inband_index(B, row, :)
		while ind > 0 && iszero(B[row, ind])
			ind -= 1
			iszero(ind) && thrown(error("last_inband_nonzero_index: No nonzero element in row $row \n"))
		end
	elseif col == n
		ind = m
	elseif row == m
...
function random_blocks_generator_no_overlap(
	...
	while round(Int64, i*δ, RoundDown) <= mn 
		...
		while lower_rows[1, i] < upper_rows[1, i] && upper_cols[1, i]  < lower_cols[1, i] 
			upper_rows[1, i] = rand(rng, δ_range)
			upper_cols[1, i] = rand(rng, δ_range)
			lower_rows[1, i] = rand(rng, δ_range)
			lower_cols[1, i] = rand(rng, δ_range)
		...
		for lb_rot_ind in 1:lb_rot_num
			...
			while lb_rot.inds <= ub_mb < lb_rot.inds + lb_rank
				...
				for ub_fill_up_row in ub_active_rows
					start_column_ind = last_inband_nonzero_index(gw.b, ub_fill_up_row-1, :)
				...
				ub_new_last_col_ind = last_inband_nonzero_index(gw.b, lb_rot.inds + lb_rank, :)
...
function residual_phase!(
	...
	for d_ind in 1:diag_size
		...
		# Regression action
		ub_g_ind > last_ub_g_ind ? ub_nb = diag_size + 1 : ub_nb = gw.b.upper_blocks[ub_g_ind].nb
		if ub_nb <= d_ind
