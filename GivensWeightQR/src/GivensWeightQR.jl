...
LinearAlgebra.ldiv!(A::QRGivensWeight, b::AbstractMatrix) =
	solve!(A, b)
...
function solve!(
  ...
  for j in Iterators.reverse(axes(A.Q.Qb, 2))
		for i in axes(A.Q.Qb, 1) 
			A.Q.Qb[i, j].inds != 0 &&	apply_inv!(A.Q.Qb[i, j], B)
		end
  end
  #Apply triangularizing rotations to b
  for j in axes(A.Q.Qt, 2)
		for i in axes(A.Q.Qt, 1)
			A.Q.Qt[i, j].inds != 0 && apply_inv!(A.Q.Qt[i, j], B)
		end
  end
  ...
  while ub_mb >= s
    ub_g_ind -= 1
    ub_g_ind == 0 ? ub_mb = 0 : ub_mb = A.R.b.upper_blocks[ub_g_ind].mb
  end
  for i in s-1:-1:1
    #Apply upper rot  
    while i == ub_mb
    ...
    for k in axes(B,2)
      B[i, k] = A.R.b[i, i]\(B[i, k] - 
				dot(view(A.R.b, i:i, i+1:last_el_in_row_ind), 
...
function random_blocks_generator(
  ...
  while round(Int64, i*δ, RoundDown) <= mn 
    ...
    upper_rows[1,i] > upper_cols[1,i] && @swap(upper_rows[1,i], upper_cols[1,i]) 
    lower_cols[1,i] > lower_rows[1,i] && @swap(lower_cols[1,i], lower_rows[1,i])
    i += 1
  end
			...
			while lb_rot.inds <= ub_mb < lb_rot.inds + lb_rank
				...
				for ub_fill_up_row in ub_active_rows
					...
					for j in  (finish_column_ind-1) : -1 : start_column_ind+1
						...
						gw.b[ub_fill_up_row, kill]=0.0 #Fix NaN issue. 
...
function residual_phase!(
	...
	ub_g_ind = 1
	while gw.b.upper_blocks[ub_g_ind].mb == 0 && ub_g_ind <= last_ub_g_ind
		ub_g_ind +=1
	end
	ub_g_ind > last_ub_g_ind  ? ub_mb = 0 : ub_mb = gw.b.upper_blocks[ub_g_ind].mb
	for d_ind in 1:diag_size
		leud = last_inband_index(gw.b, :, d_ind) #Last Element Under Diagonal (leud)
		while gw.b[leud,d_ind] == 0
			leud -= 1
		end
		qty_und_diag = leud - d_ind
		leud == m ? virtual_leud = m+1 : virtual_leud = leud
		# Regression action
		while d_ind <= ub_mb < virtual_leud
		...
		for k in qty_und_diag: -1 :1
			surv = d_ind + k - 1
			kill = d_ind + k
			rot = lgivens(gw.b[surv:kill, d_ind]..., surv)
			last_el_in_row = max(last_inband_index(gw.b, kill, :),
				last_inband_index(gw.b, surv, :))
...
function apply_rotations_forward!(
	...
	for j in axes(rot, 2)
		for i in Iterators.reverse(axes(rot, 1))
			rot[i, j].inds != 0 &&	apply!(rot[i, j], A)
		end
	end
end
