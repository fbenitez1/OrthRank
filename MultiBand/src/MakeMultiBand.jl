...
function makeMultiBand(
  n::Integer,
  blsize::Integer,
  ::Type{T},
) where {X<:AbstractFloat,T<:Union{X,Complex{X}}}
  begin
      ...
      if (blsize <= b2)
        for k = 1:(n ÷ b - 1)
          ...
          A[(j1 + b2):(j1 + b2 + blsize - 1), (k1 + b - blsize):(k1 + b - 1)] =
            randn(T, (blsize, blsize))
          A[
            (j1 + b2):(j1 + b2 + blsize - 1),
            (k1 + b2 - blsize):(k1 + b2 - 1),
          ] = randn(T, (blsize, blsize))
          j1 = j1 + b
          k1 = k1 + b
        end
